...
func (i *Watcher) do() {
	...
	watchFunc := func(options metav1.ListOptions) (watch.Interface, error) {
		timeOut := int64(60)
		return i.clientset.CoreV1().Services(metav1.NamespaceAll).Watch(context.Background(), metav1.ListOptions{TimeoutSeconds: &timeOut})
	}

	watcher, err := toolsWatch.NewRetryWatcher("1", &cache.ListWatch{WatchFunc: watchFunc})
	if err != nil {
		panic(fmt.Sprintf("could not initialise external name service watcher: %s\n", err.Error()))
	}
...
func (i *Watcher) List() ([]*corev1.Service, error) {
	list, err := i.clientset.CoreV1().Services(metav1.NamespaceAll).List(context.Background(), metav1.ListOptions{})
	if err != nil {
		return nil, errors.Wrapf(err, "could not retrieve services")
	}

	flattened := make([]*corev1.Service, 0)
	for idx, _ := range list.Items {
		flattened = append(flattened, &list.Items[idx])
	}

	log.Debug().Msgf("listed %d services", len(flattened))
...
func (i *Watcher) Relevant(item *corev1.Service) bool {
	return i.IsTypeClusterIP(item) && tools.AnnotationExists(item.GetAnnotations(), config.AnnotationExternalHostname)
}

func (i *Watcher) IsTypeExternalName(item *corev1.Service) bool {
	return item.Spec.Type == corev1.ServiceTypeExternalName
}

func (i *Watcher) IsTypeClusterIP(item *corev1.Service) bool {
	return item.Spec.Type == corev1.ServiceTypeClusterIP
}

func (i *Watcher) Delete(item *corev1.Service) error {
	return i.clientset.CoreV1().Services(item.GetNamespace()).Delete(context.Background(), item.GetName(), *metav1.NewDeleteOptions(0))
}

func (i *Watcher) Create(item *corev1.Service) error {
	_, err := i.clientset.CoreV1().Services(item.GetNamespace()).Create(context.Background(), item, *&metav1.CreateOptions{})

	return err
}

func (i *Watcher) Update(item *corev1.Service) error {
	_, err := i.clientset.CoreV1().Services(item.GetNamespace()).Update(context.Background(), item, *&metav1.UpdateOptions{})
